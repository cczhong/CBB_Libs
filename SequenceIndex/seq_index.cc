...
void SeqIndex::DumpSFA(std::string& dir, std::string& file_stem, const IDType &pivot) {
...
void SeqIndex::SetBlockConfig(const int& num_blocks, std::string &dir, std::string &file_stem) {
  ...
  if(!out_fh.good())  {
    cout << "SeqIndex::SetBlockConfig: Cannot write block-size index file: " << out_file << endl;
    exit(1);
  }
