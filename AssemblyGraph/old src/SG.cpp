...
unsigned int goodMapping(std::string& CIAGR, int flag)
{
  ...
  if((flag&256) == 256)             return 0x80000000;  // not primary alignment
  if((flag&2048) == 2048)           return 0x80000000;  // supplementary alignment
  if(match < 100)                   return 0x40000000;  // mapping length too short
  if(symbols.size() > 2)            return 0x40000000;  // too fragmented
  if (symbols.size() == 2)          return 0x20000000;
  return match;
}
